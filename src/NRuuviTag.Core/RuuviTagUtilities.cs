...
/// <summary>
        /// Creates a new <see cref="RuuviTagSample"/> from a <paramref name="payload"/> that uses 
        /// RuuviTag's Encrypted format (data format 8).
        /// </summary>
        /// <param name="timestamp">
        ///   The timestamp for the sample.
        /// </param>
        ...
        public static RuuviTagSample CreateSampleFromEncryptedPayload(DateTimeOffset timestamp, double signalStrength, Span<byte> payload) {
            if (payload == null) {
                throw new ArgumentNullException(nameof(payload));
            }
            if (payload.Length < 24) {
            ...
            if (payload[0] != Constants.DataFormatEncryptedEnvironmental) {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Resources.Error_UnexpectedDataFormat, Constants.DataFormatRawV2, payload[0]), nameof(payload));
            }
            ...
            try {
                var result = new RuuviTagSample() {
                    Timestamp = timestamp,
                    SignalStrength = signalStrength,
                    DataFormat = Constants.DataFormatEncryptedEnvironmental
                };
        ...
        public static RuuviTagSample CreateSampleFromPayload(DateTimeOffset timestamp, double signalStrength, Span<byte> payload) {
            ...
            switch (payload[0]) {
                ...
                case Constants.DataFormatEncryptedEnvironmental:
                    return CreateSampleFromEncryptedPayload(timestamp, signalStrength, payload);
                default:

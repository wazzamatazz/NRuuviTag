...
`RuuviTagSample` has been converted from a mutable class to an immutable record with `init`-only properties.
...
### 5. `RuuviTagListener` API Changes

The `RuuviTagListener` base class has been updated to apply device filtering at the listener level rather than the publisher level. A new `IDeviceResolver` interface has been introduced to allow a listener to determine if an advertisement has been received from a known Ruuvi device.

The abstract `ListenAsync` method has been removed and replaced with a new `RunAsync` method that signals to the implementation to start listening for advertisements. Implementations call a new `DataReceived` method to notify the base class about new advertisement data.

The `IRuuviTagListener.ListenAsync` overload that accepted a filter function has been removed. Filtering is now controlled directly by the listener rather than by individual subscribers.
...
```csharp    
protected RuuviTagListener(
    RuuviTagListenerOptions options, 
    IDeviceResolver? deviceResolver = null, 
    TimeProvider? timeProvider = null, 
    ILogger<RuuviTagListener>? logger = null
) { ... }
```

**Impact:**
- Filtering is now performed early at the listener level
- Simplified listener implementations


#### `ListenAsync` Method Removal

**Old Behaviour (v3.0.0):**
```csharp
protected abstract IAsyncEnumerable<RuuviTagSample> ListenAsync(
...
### 6. `RuuviTagPublisher` API Changes
...
- The `filter` parameter has been removed - filtering is now performed at the listener level
...
If you have custom publisher implementations, update the constructor parameters and update the `RunAsync` method:
...
### 7. JSON Property Name Attributes

All properties on `RuuviDataPayload` and `RuuviTagSample` now have explicit `[JsonPropertyName]` attributes to ensure consistent JSON serialization regardless of serializer settings.
...
- **Data Format E1 (Extended V1)**: New format used by Ruuvi Air
- **Data Format 6**: Truncated format used by Ruuvi Air for compatibility with receivers that do not support Bluetooth 5.0 extended advertisements

**New Properties:**
- `Calibrated` (`bool?`): Indicates whether sensor values have been factory-calibrated (Extended V1 only)
- `DataFormat` (`byte?`): Identifies the data format version (`0x05` for Raw V2, `0xE1` for Extended V1, `0x06` for Truncated)
...
**Example Usage:**
```csharp
var listener = new WindowsSdkListener(new WindowsSdkListenerOptions());
...
4. Update custom listener implementations if needed
5. Update custom publishers to use the new `ChannelReader`-based `RunAsync` signature
6. Replace property mutations with `with` expressions
